...
class SACWrapper:
    def __init__(
        ...
        # Try to load the SAC model
        try:
            print("Attempting to load SAC model...")
            self.model = SAC.load(model_path)
            print("✅ Successfully loaded trained SAC model!")
            
        except Exception as e:
            print(f"Failed to load SAC model: {e}")
            print("Attempting to create new SAC model and load policy weights...")
            
            # Create a new SAC model with correct architecture first
            ...
            try:
                ...
                with zipfile.ZipFile(model_path, 'r') as zip_file:
                    ...
                    print("✅ Successfully loaded SAC policy weights from saved model")
                    
            except Exception as e2:
                print(f"Failed to load SAC policy weights: {e2}")
                print("⚠️ Using untrained SAC model - the car will drive randomly")
                self.model = None
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        if self.model is not None:
            # Use the SAC model if available
...
def main(
    model_path: str = "models/sac_f1_qualifying_2025-10-18_21-49-56/final_model.zip",
    ...
    if video_recording:
        ...
        if not os.path.exists("videos"):
            os.makedirs("videos")
        video_path = os.path.join("videos", f"SAC_{time.time()}")
        env = gym.wrappers.RecordVideo(env, video_path)

    # SAC model
    sac_wrapper = SACWrapper(model_path, original_env=env, horizon=30)
