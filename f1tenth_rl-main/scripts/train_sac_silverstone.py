...
def create_env(n_envs: int = 4):
    try:
        ...
        return DummyVecEnv([make_silverstone_env(0, seed)])
...
def main():
    ...
    model_dir = f"models/sac_silverstone_{run_id}"
    os.makedirs(model_dir, exist_ok=True)
    os.makedirs(f"runs/sac_silverstone_{run_id}", exist_ok=True)
    ...
    try:
        model = SAC(
            ...
            tensorboard_log=f"runs/sac_silverstone_{run_id}",
            device=device,
            seed=seed,
    ...
    try:
        ...
        print(f"TensorBoard logs: runs/sac_silverstone_{run_id}")
