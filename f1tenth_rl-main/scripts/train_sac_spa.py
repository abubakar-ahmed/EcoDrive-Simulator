...
def make_spa_env(rank: int = 0, seed: int = 0):
    """Create environment fixed to Spa"""
...
def create_env(n_envs: int = 4):
    try:
        if n_envs == 1:
            env = DummyVecEnv([make_spa_env(0, seed)])
        else:
            env = SubprocVecEnv([make_spa_env(i, seed) for i in range(n_envs)])
        return env
    except Exception as e:
        print(f"Failed to create parallel environments: {e}")
        return DummyVecEnv([make_spa_env(0, seed)])

def main():
    n_envs = 4
    env = create_env(n_envs)
    eval_env = DummyVecEnv([make_spa_env(0, seed + 1000)])

    model_dir = f"models/sac_spa_{run_id}"
    os.makedirs(model_dir, exist_ok=True)
    os.makedirs(f"runs/sac_spa_{run_id}", exist_ok=True)

    try:
        model = SAC(
            sac_config["policy_type"],
            env,
            ...
            use_sde_at_warmup=sac_config["use_sde_at_warmup"],
            learning_rate=sac_config["learning_rate"],
            verbose=sac_config["verbose"],
            tensorboard_log=f"runs/sac_spa_{run_id}",
            device=device,
            seed=seed,
    ...
    try:
        ...
        print(f"TensorBoard logs: runs/sac_spa_{run_id}")
